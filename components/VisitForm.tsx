...
export function VisitForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setStatus({ type: "loading", message: "Envoi de votre demande…" });
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.message || "La demande n’a pas pu être transmise.");
      form.reset();
      setStatus({ type: "success", message: result.message });
    } catch (error) {
      setStatus({
        type: "error",
        message: error instanceof Error ? error.message : "La demande n’a pas pu être transmise.",
      });
    }
  }
  ...
  return (
    <form className="visit-form" onSubmit={submit} noValidate>
      <div className="form-grid two-columns">
        <label>
          <span>Prénom <b aria-hidden="true">*</b></span>
          <input name="firstName" autoComplete="given-name" required minLength={2} maxLength={80} />
        </label>
        <label>
          <span>Nom <b aria-hidden="true">*</b></span>
          <input name="lastName" autoComplete="family-name" required minLength={2} maxLength={80} />
        </label>
      </div>
      <div className="form-grid two-columns">
        <label>
      ...
      <div className="form-grid two-columns">
        <label>
          <span>Nature du projet <b aria-hidden="true">*</b></span>
        ...
        <label>
          <span>Délai envisagé <b aria-hidden="true">*</b></span>
      ...
      <label>
        <span>Votre message <b aria-hidden="true">*</b></span>
        <textarea
          name="message"
          rows={6}
          required
